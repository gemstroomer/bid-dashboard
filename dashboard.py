...
with st.sidebar:
    ...
    # Dates are auto-detected from the uploaded file — you can still adjust manually
    _def_start = detected_start if "detected_start" in vars() and pd.notna(detected_start) else pd.Timestamp("2026-05-18")
    _def_end   = detected_end   if "detected_end"   in vars() and pd.notna(detected_end)   else pd.Timestamp("2026-06-21")
    period_start = st.date_input("Start", value=_def_start)
    period_end   = st.date_input("End",   value=_def_end)
    st.caption("Auto-detected from file — adjust if needed")
...
def load_data(f_ab, f_bo):
    # Auto-detect period from row 4, column C of the raw file
    try:
        raw = pd.read_excel(f_ab, header=None, nrows=5)
        period_str = str(raw.iloc[3, 2])
        parts = period_str.strip().split("-")
        detected_start = pd.to_datetime(parts[0], format="%d%b%Y", errors="coerce")
        detected_end   = pd.to_datetime(parts[1], format="%d%b%Y", errors="coerce")
    except Exception:
        detected_start = pd.Timestamp("2026-05-18")
        detected_end   = pd.Timestamp("2026-06-21")

    df_ab = pd.read_excel(f_ab, skiprows=range(0, 8))
    df_bo = pd.read_excel(f_bo, skiprows=range(0, 8))
    df_ab["ac_type"] = "AB"
    df_bo["ac_type"] = "BO"
    return pd.concat([df_ab, df_bo], ignore_index=True), detected_start, detected_end

df_raw, detected_start, detected_end = load_data(f_ab, f_bo)
